...
if ($_SERVER['SCRIPT_NAME'] !== DIR_WS_ADMIN . (!strstr(FILENAME_LOGIN, '.php') ? FILENAME_LOGIN . '.php' : FILENAME_LOGIN) && $_SERVER['SCRIPT_NAME'] !== DIR_WS_ADMIN . (!strstr(FILENAME_LOGOFF, '.php') ? FILENAME_LOGOFF . '.php' : FILENAME_LOGOFF)) {
    ...
    $module_constant = 'ZEN_COLORBOX'; // This should be a UNIQUE name followed by _VERSION for convention
    $module_installer_directory = DIR_FS_ADMIN . 'includes/installers/zen_colorbox'; // This is the directory your installer is in, usually this is lower case
    $module_name = "Zen Colorbox"; // This should be a plain English or Other in a user friendly way
    $admin_page = 'ZenColorbox';
    $zencart_com_plugin_id = 1322; // from zencart.com plugins - Leave Zero not to check
                    ...
                    if (substr($installer, strrpos($installer, '.')) == $file_extension && (preg_match('~^[^\._].*\.php$~i', $installer) > 0 || $installer != 'empty.txt')) {
                        if (version_compare($newest_version, substr($installer, 0, -1 * $file_extension_len)) >= 0 && version_compare($current_version, substr($installer, 0, -1 * $file_extension_len)) < 0) {
                            ...
                            if ($cancel_install === true) {
                                $messageStack->add("Stopped installation of " . $module_name . " v" . $current_version, 'warning');
                                break;
                            }
